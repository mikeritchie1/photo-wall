...
- The deployed wall reads manifests, groups, and metadata sidecars from GitHub, while the actual image and video files are served from the Cloudflare R2 bucket. Audio remains in the GitHub Pages `audio/` folder. Keep the `images/` and `videos/` folder structure identical in GitHub and R2, but only upload the actual media files to R2.
